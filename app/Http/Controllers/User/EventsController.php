...
class EventsController extends Controller
{
    public function index(){
        ...
        if($userId){
            $event = Event::where('userId', $userId)->orderBy('updated_at','DESC')->paginate(10);
        }else{
            $event = null;
        }
        ...
        return view('user.event.index',compact('event','category','modelEventCategory'));
    }
    ...
    public function eventDetails(Request $request,$id){ 
        ...
        return  view('user.event.event-details',compact('eventDetails','allModelsEvent'));
    }
    ...
    public function modelEventCategory(Request $request){
        ...
        return view('user.event.categoryModelDetails',compact('modelEventCategory'));
    }
    ...
    public function list(Request $request){
       $draw = $request->get('draw');
        ...
        if ($search != "") {
            $members = $members->where('userId', $userId)->where(function ($q) use ($search) {
        ...
        if (!isset($request->order)) {
            $members = $members->where('userId', $userId)->orderby('updated_at', 'DESC');
        } else {
    ...
    public function edit(Request $request,$id){
        try {
            ...
            return view('user.event.create',compact('event','modelList','category','userIds','modelEventCategory'));
        }catch (Exception $e) {
